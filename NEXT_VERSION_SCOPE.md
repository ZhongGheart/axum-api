...
本计划已按 PR 粒度执行完毕，分支 `v0.2.0`，共 4 个提交：

| 提交 | 范围 | 状态 |
|---|---|---|
| `2898a306` | PR-1 可部署：启动迁移、真实健康探活、容器与编排加固 | ✅ 完成 |
| `0718456c` | PR-2 安全：单令牌注销、登录爆破防护、删除负收益安全模块 | ✅ 完成 |
| `6282f394` | PR-3 权限数据：角色单一来源、事务化、审计接线、清除死代码 | ✅ 完成 |
| `b2ec1c57` | PR-4 验证：集成测试、前端单测、CI 强门禁、文档对齐 | ✅ 完成 |

**验证证据（本地真实 Postgres 15 + Redis 6.2）**

| 检查 | 结果 |
|---|---|
| `cargo fmt --all --check` | ✅ 通过 |
| `cargo clippy --all-targets --all-features -- -D warnings` | ✅ 0 warning（原 35） |
| `cargo test`（单元） | ✅ 24 passed |
| `cargo test --test api_integration -- --ignored` | ✅ 10 passed（空库迁移 / 会话 / 权限 / 一致性 / 文档契约） |
| `pnpm lint` | ✅ 0 error（原 `.eslintrc.cjs` 在 ESLint 9 下完全不可用） |
| `pnpm typecheck` / `pnpm test` / `pnpm build` | ✅ 通过 / 9 passed / 通过 |
| 空库启动 + `/api/health` | ✅ 自动建表并返回 200；Redis 掉线时正确返回 503 |
| `docker compose config` | ✅ 通过（`DB_PASSWORD`/`JWT_SECRET` 缺失时按预期报错） |

**与原计划的偏差（均为主动取舍，已记录理由）**

1. **OpenAPI 未改为代码生成**：`utoipa` 迁移需为 36 条路由逐一标注，收益/风险比不划算。
   改为「文档中每条路由都必须真实存在」的集成测试（已用注入幽灵路由验证该测试会失败）。
2. **未引入独立权限码体系**：当前只有 admin/user 两个内置角色，
   权限码不产生额外强制力；改为在 README 中诚实标注"按钮级权限为基于角色的显隐"。
3. **审计日志未做自动保留/清理**：属新增运维机制，缺少当前需求支撑，列入已知限制。
4. **业务错误码枚举未引入**：会把 `code` 从"等于 HTTP 状态码"改成业务码，
   属破坏性契约变更；本版改为修正错误状态码语义并统一响应体。
5. **前端未做全量 Prettier 格式化**：50 个文件风格不一致，一次性重排会淹没真实改动，未纳入 CI。

**未能执行的环境相关项**：本机 Docker daemon 未运行，因此
`docker build` 与 `docker compose up` 端到端冒烟未在本地实跑（已写入 CI 的 docker job）。
等价路径已用本地真实 Postgres/Redis 验证：空库自动迁移 + 健康探活 + 完整接口链路。
