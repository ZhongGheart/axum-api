<template>
  <div class="monitor-page">
    ...
    <!-- 告警信息 -->
    <n-card title="告警信息" size="small" v-if="alerts.length > 0">
      <n-timeline>
        <n-timeline-item v-for="(a, i) in alerts" :key="i" :type="a.level === 'critical' ? 'error' : 'warning'"
          :content="a.message" />
      </n-timeline>
    </n-card>
  </div>
...
const columns: DataTableColumn[] = [
  { title: '#', key: 'index', width: 50, render(_: unknown, index: number) { return index + 1 } },
  { title: '方法', key: 'method', width: 80,
    render(row: Record<string, unknown>) {
      const method = row.method as string
      const color = method === 'GET' ? 'success' : method === 'POST' ? 'primary' : method === 'PUT' ? 'warning' : 'error'
      return h(NTag, { type: color as 'success' | 'primary' | 'warning' | 'error', size: 'tiny' }, () => method)
    }
  },
  { title: '路径', key: 'path', ellipsis: { tooltip: true } },
  { title: '调用次数', key: 'call_count', width: 90, sorter: (a: Record<string, unknown>, b: Record<string, unknown>) => (a.call_count as number) - (b.call_count as number) },
  { title: '错误数', key: 'error_count', width: 80, sorter: (a: Record<string, unknown>, b: Record<string, unknown>) => (a.error_count as number) - (b.error_count as number) },
  { title: '平均(ms)', key: 'avg_duration_ms', width: 90, sorter: (a: Record<string, unknown>, b: Record<string, unknown>) => (a.avg_duration_ms as number) - (b.avg_duration_ms as number) },
...
async function fetchData() {
  ...
  try {
    ...
    const data = res as unknown as { metrics: ApiMetric[]; summary: Record<string, unknown> }
    metrics.value = data.metrics
    Object.assign(summary, data.summary)

    const alertRes = await monitorApi.getAlerts()
    const alertData = alertRes as unknown as { alerts: AlertItem[] }
